...
sleep_time = (
    60 * 10
)  # in seconds, between twitter API calls. Default rate limit is 180 per 15 minutes
    ...
    for days in [1, 7, 30]:
        ...
        for tweet in relevant:
            ...
            for pid in tweet["pids"]:
                ...
                tweet_sort_bonus = (
                    10000 if isok2 else 0
                )  # lets bring meaningful comments up front.
                records_dict[pid]["tweets"].append(
                    {
                        "screen_name": tweet["user_screen_name"],
                        "image_url": tweet["user_image_url"],
                        "text": tweet["text"],
                        "weight": float_vote + tweet_sort_bonus,
                        "ok": isok2,
                        "id": str(tweet["id"]),
                    }
                )
