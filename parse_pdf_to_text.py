...
if not shutil.which("pdftotext"):  # needs Python 3.3+
    print(
        "ERROR: you don't have pdftotext installed. Install it first before calling this script"
    )
    sys.exit()
...
for i, f in enumerate(
    ...
    if not os.path.isfile(txt_path):
        ...
        print(
            "there was a problem with parsing %s to text, creating an empty text file."
            % (pdf_path,)
        )
        os.system(
            "touch " + txt_path
